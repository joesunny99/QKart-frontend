...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
        ...
        let data = {"address":newAddress.value};
        let response = await axios.post(`${config.endpoint}/user/addresses`, data, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
    ...
    try{
      if(validateRequest(items, addresses)){              
        let data = {"addressId":addresses.selected};
          let response = await axios.post(`${config.endpoint}/cart/checkout`, data, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });          
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>                                                          
              {
                addresses.all.length? (                           
                    addresses.all.map(add=>{
                      return(
                      <Box className={`address-item ${add._id === addresses.selected? 'selected': 'not-selected'}`} 
                        key={add._id} 
                        onClick={e=>{setAddresses({...addresses, selected:add._id})}}>
                        <Typography>{add.address}</Typography> 
                      <Button onClick={async(e)=>await deleteAddress(token, add._id)} startIcon={<DeleteIcon />} >Delete</Button>                        
                      </Box>
                      )                  
                      })                                 
                  ):(                    
            ...
            <Button
              ...
              variant="contained"
              onClick={async(e)=>{await performCheckout(token, items, addresses)}}
            >
              PLACE ORDER
            </Button>
          </Box>
